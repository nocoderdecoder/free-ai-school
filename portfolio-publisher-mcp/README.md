...
- `create_next_staged_lab_card_publish_packet`: turns that deterministic selection and live rehearsal into an owner-friendly Markdown packet with the exact handoff and patch files to review, current route/screenshot/icon evidence, and the safe validate/apply/smoke sequence. It is read-only and token-free.
...
For a copy-ready owner handoff, call `create_next_staged_lab_card_publish_packet`. It selects the same first publish-ready card and combines queue context, staged card details, exact patch/handoff paths, live readiness evidence, and the rehearsal sequence in Markdown. An empty packet provides recovery guidance instead of inventing a selection. The packet never changes files or includes a `reviewToken`; validation must still happen immediately before controlled apply.
