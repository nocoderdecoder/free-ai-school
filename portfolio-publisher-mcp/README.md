...
- `list_staged_lab_card_patches`: lists pending staged patch artifacts with their current review status, live route/screenshot/icon readiness, file paths, and owner next step. It recomputes readiness from current local files, keeps the handoff's historical readiness as a separate field, and identifies incomplete `.patch`/Markdown pairs without changing files.
