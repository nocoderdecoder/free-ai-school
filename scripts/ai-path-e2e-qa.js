globalThis.__AI_PATH_QA_RUN__ = async (page) => {
  ...
  const outcomeGroupName = /Which outcomes matter to you right now\?Choose all that apply/
  const outcomeGroup = page.getByRole('group', { name: outcomeGroupName })
  await choose(outcomeGroupName, 'Use AI better in my everyday work')
  await choose(outcomeGroupName, 'Save time by automating repeated work')
  const selectedOutcomes = outcomeGroup.getByRole('checkbox', { checked: true })
  assert(await selectedOutcomes.count() === 2, 'capability direction did not retain multiple selected outcomes')
  await choose(outcomeGroupName, 'Help me discover what would suit me')
  assert(await selectedOutcomes.count() === 1 && await outcomeGroup.getByLabel(/Help me discover/).isChecked(), 'discovery did not clear conflicting concrete outcomes')
  await choose(outcomeGroupName, 'Use AI better in my everyday work')
  await choose(outcomeGroupName, 'Save time by automating repeated work')
  assert(await selectedOutcomes.count() === 2 && !(await outcomeGroup.getByLabel(/Help me discover/).isChecked()), 'concrete outcomes did not replace the exclusive discovery choice')
  ...
  await supportedEvidence.getByLabel('Automation and integrations', { exact: true }).check()
  ...
  assert(await capabilityResult.getByText('Adapted practice', { exact: true }).first().isVisible(), 'capability result omitted calibrated evidence')
