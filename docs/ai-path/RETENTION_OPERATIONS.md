...
The retention service purges expired plan records first, then sessions. Each target has a fixed 20-second application deadline so the two RPCs fit within the 60-second scheduler boundary. A timeout is treated as an unknown commit and returns only the bounded `purge_failed` code. Both operations must be idempotent because a retry can follow a timeout or partial failure, and session deletion can cascade already-expired plans. Operational events contain only the target name, delete count, opaque run ID, time, and bounded error code—never transcript, check-in, goal, role, report narrative, or database error text.
...
3. Assemble the implemented server-only purge adapter in isolated staging without placing a service credential in user routes or logs.
