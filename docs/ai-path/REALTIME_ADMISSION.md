...
- late finalization for seven days so expired leases can record usage already incurred without retaining replay state forever;
...
All lifecycle and maintenance mutations take the same transaction-scoped PostgreSQL advisory lock. User-facing reserve, finalize, and cancel never perform a global expiry write. Reserve fails closed when any stale reserved row remains, so the bounded maintenance runner must drain expiry backlog before new admission. Finalize and cancel lock and inspect only their target row: finalization converts a stale target to expired and permits reconciliation only inside the fixed seven-day window, while cancellation converts a stale target to expired and rejects it. Because the shared lock is acquired before every decision, these target transitions and bounded maintenance cannot race admission checks.
...
- Expiry releases concurrency and the estimate. Late finalization remains allowed for seven days because expiry cannot erase provider usage that already happened; after that fixed reconciliation window the operation fails closed.
...
- Migration `20260717070000` defines a dormant 90-day terminal ledger/idempotency retention policy and content-free daily accounting archive. Its bounded maintenance behavior still requires real-database proof before activation.
