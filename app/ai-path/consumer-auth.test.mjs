...
test('route and proxy boundaries use verified identity and mutation-safe verbs', async () => {
  ...
  assert.match(signInSource, /form\.get\('remember'\) === REMEMBER_SEARCH_VALUE/)
  assert.match(signInSource, /callback\.searchParams\.set\('remember', REMEMBER_SEARCH_VALUE\)/)
  ...
  assert.match(googleSource, /form\.get\('remember'\) === REMEMBER_SEARCH_VALUE/)
  assert.match(googleSource, /callback\.searchParams\.set\('remember', REMEMBER_SEARCH_VALUE\)/)
  ...
  assert.match(callbackSource, /requestUrl\.searchParams\.get\('remember'\) === REMEMBER_SEARCH_VALUE/)
  assert.match(callbackSource, /createConsumerAuthRequestContext\(request, \{ remember \}\)/)
})
...
test('consumer pages distinguish configured auth from a verified signed-in user', async () => {
  ...
  assert.match(serverSource, /supabaseAuthCookieOptions\(process\.env\.NODE_ENV, \{ remember \}\)/)
  assert.match(pageSource, /user = authConfigured \? await getVerifiedConsumerUser\(\) : null/)
  assert.match(pageSource, /authenticatedExperienceEnabled = Boolean\(user\)/)
  assert.match(pageSource, /consumerUser=\{user \? \{ email: user\.email \} : null\}/)
